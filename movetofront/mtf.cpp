/**
 * Move to Front Algorithm(Source file)
 * 2019-02-02
 */

#include <algorithm>
#include <iostream>
#include <vector>

#include "mtf.h"

/* Exposed some Elements in STL for this File */
using std::find;
using std::is_same;
using std::vector;

/* Anonymous Namesapce */
namespace
{
	/* the ASCII List for Move-To Front(includes the extended partition) */
	static const vector<uint8_t> __ascii 
	{
		,
	};

	/* A Cache for ASCII List */
	static vector<uint8_t> __dict(256);
}
/**
 * @brief	the Move-To Front Function
 * @param	const uint8_t* nString
 * 				the string will be processed by MTF
 * @param   uint32_t nSize
 *              Length of Source Array
 * @param   uint8_t* rArray
 *                 result after MTF processing
 * @retval	bool
 * @note	None
 */
bool NXZIP::NXZ_MoveToFront(const uint8_t* nString, uint32_t nSize, uint8_t* rArray)
{
    // Parameters Check
    if(nullptr == rArray || 0 == nSize || nullptr == nString)
    {
        return false;
    }

    // directionary reload
    ::__dict = ::__ascii;

    // iterator of vector<uint8_t>
    auto it = ::__dict.begin();

    // Move-To Front
    for(uint32_t i = 0; i < nSize; i++)
    {
        if(*(nString+i) == __dict.front())
        {
            *(rArray+i) = 0;
            continue;
        }

        // Get the Index in ASCII List
        it = find(__dict.begin(), __dict.end(), *(nString+i));
        *(rArray+i) = (uint8_t)(it - __dict.begin());

        // Move the Character to the top of ASCII List
        __dict.erase(it, it+1);
        __dict.insert(__dict.begin(), *(nString+i));
    }

    return true;
}

/**
 * @brief	the Inverse Move-To Front Function
 * @param   const uint8_t* rArray
 *              Source Array to process by IMTF
 * @param   uint32_t nSize
 *              Length of Source Array
 * @param	uint8_t* nString
 * 				the result string after processed by MTF, must be vector<uint8_t> or vector<unsigned char>
 * @retval	bool
 * @note	None
 */
bool NXZIP::NXZ_MoveToFront_Inverse(const uint8_t* nArray, uint32_t nSize, uint8_t* rString)
{
    // Parameters Check
    if(nullptr == nArray || 0 == nSize || nullptr == rString)
    {
        return false;
    }

    // directionary reload
    __dict = __ascii;

    // iterator of vector<uint8_t>
    auto it = __dict.begin();

    // Inverse Move-To Front
    for(uint8_t i = 0; i < nSize; i++)
    {
        // Get the Characters in dictionary
        *(rString+i) = __dict.at(*(nArray+i));

        // calculate for iterator
        it += *(nArray+i);

        // Move the Character that copied to rString to top of dictionary
        __dict.erase(it, it+1);
        __dict.insert(__dict.begin(), *(rString+i));

        // reset the iterator
        it = __dict.begin();
    }

    return true;
}

//EOF
